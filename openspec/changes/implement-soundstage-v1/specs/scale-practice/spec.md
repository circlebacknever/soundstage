...
- **WHEN** the scale setup route is displayed
- **THEN** the chip row contains "Major", "Minor", "Pentatonic", "Blues", and "Dorian", wraps as needed, uses `--paper-soft` with hairline border and `--ink-2` for default chips, and uses `--peri` with white text for the selected chip

#### Scenario: Root key picker renders
- **WHEN** the scale setup route is displayed
- **THEN** the root picker shows a seven-column grid for C, D, E, F, G, A, and B, with default keys using `--paper-soft`, Fraunces 600 at 24px, and selected key using `--peri` with white text
...
- **THEN** hit cells use `--mint` filled background with white text, next cells use `--paper` background with 3px `--coral` outline and `--coral-ink` text in Nunito 800, and remaining scale cells use the peri dot state
